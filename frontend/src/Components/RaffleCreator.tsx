        ...
        .then((data) => {
          ...
          for (let i = 0; i <= data.result.length; i++) {
            if (tempTokens.length > 0) {
              try {
                let index = tempTokens.findIndex(
                  (temp) =>
      ...
      for (let i = 0; i <= tempTokens.length; i++) {
        ...
        if (tempTokens[i]) {
          ...
          let contract = new ethers.Contract(
            tempTokens[i].contractAddress,
            _abi,
            signer
          );
          let metaData = await contract.tokenURI(
            parseInt(tempTokens[i].tokenID)
          );
          fetch(metaData)
            .then((res) => {
              return res.json();
      ...
      <div className="PopUpRaffle-Form">
        ...
        <div className="CreateRaffle-Form-Container">
          <form className="CreateRaffle-Form" onSubmit={(e) => handleSubmit(e)}>
            ...
            {/* <TextField
              className="RaffleForm-Minimum-Buyin"
              defaultValue="0.08"
              id="filled-basic"
              label="Deposit"
              variant="filled"
            ></TextField> */}
            <h3 className="Minimum-BuyIn-h3">
              Minimum Tickets: (1 ticket = 0.01)
            </h3>
            {/* <TextField
              className="RaffleForm-Minimum-Tickets"
              defaultValue="16"
              id="filled-basic"
              label="Deposit"
              variant="filled"
