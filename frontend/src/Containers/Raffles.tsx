    ...
    const getRaffles = async () => {
        if (window.ethereum) {
            ...
            for (let i =0; i<=rafflesLength-1; i++ ) {
                ...
                var tempRaffle:any  = {'contractAddress': raffle.contractAddress, 'tokenImage': raffle.tokenImage, 'creatorAddress': gameInfo.creatorAddress,
                 'buyInPrice': parseInt(gameInfo.buyInPrice, 16), 'winner': gameInfo.winner, 'collectionName': gameInfo.collectionName, 'tokenID': parseInt(gameInfo.tokenID, 16)}
                if (gameInfo.winner === "0x0000000000000000000000000000000000000000") { // Will update when refactor Raffles contract
                    setCurrentRaffles((currentRaffles: any) => [...currentRaffles, tempRaffle])
                }else {
